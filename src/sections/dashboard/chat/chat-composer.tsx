...
const useRecipients = () => {
  ...
  const onRecipientAdd = useCallback((recipient: Contact): void => {
  ...
  const onRecipientRemove = useCallback((recipientId: string): void => {
    setRecipients((prevState) => {
...
export const ChatComposer: FC<ChatComposerProps> = (props) => {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flexGrow: 1 }} />
      <Divider />
      <ChatMessageAdd disabled={!canAddMessage}
onSend={handleSend} />
    </Box>
  )
}
